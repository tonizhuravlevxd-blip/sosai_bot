...
async def fal_generate(model, prompt, images=None, max_wait=FAL_IMAGE_MAX_WAIT):
    """
...
cancel_button = InlineKeyboardMarkup([
    [InlineKeyboardButton("❌ Отменить", callback_data="finish")]
])
            ...
            try:
                if status:
                    await status.edit_text(
                        ...
                        "Попробуйте ещё раз позже или упростите запрос."
                    )
            except Exception:
                pass
